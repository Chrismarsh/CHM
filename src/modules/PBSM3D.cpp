        ...
        : module_base("PBSM3D", parallel::domain, cfg)
{
    ...
    if ((use_exp_fetch && use_PomLi_probability) || (use_tanh_fetch && use_PomLi_probability))
        BOOST_THROW_EXCEPTION(module_error() << errstr_info(
                "PBSM3d: Cannot specify both exp_fetch/tanh_fetch and use_PomLi_probability"));
...
void PBSM3D::init(mesh &domain)
{
    ...
    settling_velocity = cfg.get("settling_velocity",
                                0.5); // m/s, Lehning, M., H. Löwe, M. Ryser, and N. Raderschall (2008), Inhomogeneous precipitation distribution and snow transport in steep terrain, Water Resour. Res., 44(7), 1–19, doi:10.1029/2007WR006545.
...
void PBSM3D::run(mesh &domain)
{
    ...
    viennacl::vector_base<vcl_scalar_type> init_temporary(vl_C.handle(),
                                                          viennacl::compressed_matrix<vcl_scalar_type>::size_type(
                                                                  nnz + 1), 0, 1);
    // write:
    ...
    //get row buffer
    unsigned int const *row_buffer = viennacl::linalg::host_based::detail::extract_raw_pointer<unsigned int>(
            vl_C.handle1());
    unsigned int const *col_buffer = viennacl::linalg::host_based::detail::extract_raw_pointer<unsigned int>(
            vl_C.handle2());
    vcl_scalar_type *elements = viennacl::linalg::host_based::detail::extract_raw_pointer<vcl_scalar_type>(
            vl_C.handle());
...
#pragma omp for
            ...
            oe.Run([&]
                   {
                       ...
                       double u10 = Atmosphere::log_scale_wind(uref, Atmosphere::Z_U_R, 10,
                                                               snow_depth); //used by the pom probability forumuation, so don't hide behide debug output
                       if (debug_output) (*face)["U_10m"_s] = u10;


                       double swe = (*face)["swe"_s]; // mm   -->    kg/m^2
                       swe = is_nan(swe) ? 0
                                         : swe; // handle the first timestep where swe won't have been updated if we override the module order
                       ...
                       if (height_diff <= cutoff && swe >= min_mass_for_trans && !is_water(face))
                       {
                           ...
                           auto ustarFn = [&](double ustar) -> double
                           {
                               ...
                               return PhysConst::kappa * uref /
                                      log(Atmosphere::Z_U_R / (0.6131702344e-2 * ustar * ustar + 0.5 * lambda)) - ustar;
                           };
                       ...
                       if (d->saltation)
                       {
                           ...
                           } else if (use_PomLi_probability) // Pomeroy and Li 2000 upscaled probability
                           {
                               ...
                               double u_mean =
                                       11.2 + 0.365 * T + 0.00706 * T * T + 0.9 * log(A); // eqn 10  T -> air temp, degC
                       ...
                       for (int z = 0; z < nLayer; ++z)
                       {
                           ...
                           {
                               u_z = std::max(0.01,
                                              Atmosphere::log_scale_wind(uref, Atmosphere::Z_U_R, cz, snow_depth,
                                                                         d->z0));
                           }
                           ...
                           double D = 2.06e-5 * pow(t / 273.15,
                                                    1.75); //diffusivity of water vapour in air, t in K, eqn A-7 in Liston 1998 or Harder 2013 A.6

                           // (A.9)
                           double lambda_t = 0.000063 * t +
                                             0.00673; //  thermal conductivity, user Harder 2013 A.9, Pomeroy's is off by an order of magnitude, this matches this https://www.engineeringtoolbox.com/air-properties-d_156.html
                            ...
                            */
                               ...
                               auto fx = [=](double Ti)
                               {
                                   return boost::math::make_tuple(
                                           ...
                                           D * L *
                                           (-0.6110000000e-3 * mw *
                                            (17.3 / (237.3 + Ti) - 17.3 * Ti / pow(237.3 + Ti, 2)) *
                                            exp(17.3 * Ti / (237.3 + Ti)) / (R * (Ti + 273.15)) +
                                            0.6110000000e-3 * mw * exp(17.3 * Ti / (237.3 + Ti)) /
                                            (R * pow(Ti + 273.15, 2))) /
                                           lambda_t - 1);
                               };
                           ...
                           } else // Use PBSM. Eqn 11 Pomeroy, Gray, Ladine, 1993  "﻿The prairie blowing snow model: characteristics, validation, operation"
                           {
                               ...
                               double sigma =
                                       (rh - 1.0) *
                                       (1.019 + 0.027 * log(cz)); //undersaturation, Pomeroy and Li 2000, eqn 14
                               ...
                               double Qr = 0.9 * M_PI * sqrt(rm) *
                                           120.0; // 120.0 = PBSM_constants::Qstar (Solar Radiation Input), no idea where 0.9 comes from

                               // eqn 11 in PGL 1993
                               dmdtz = Sh * rho * D *
                                       (6.283185308 * Nu * R * rm * sigma * t * t * lambda_t - L * M * Qr +
                                        Qr * R * t) /
    ...
    viennacl::linalg::chow_patel_ilu_precond<viennacl::compressed_matrix<vcl_scalar_type> > chow_patel_ilu(vl_C,
                                                                                                           chow_patel_ilu_config);
...
#pragma omp parallel for
        ...
        oe.Run([&]
               {
                   ...
                   for (int z = 0; z < nLayer; ++z)
                   {
                       ...
                       c = c < 0 || is_nan(c) ? 0
                                              : c; //harden against some numerical issues that occasionally come up for unknown reasons.
    ...
    //get row buffer
    unsigned int const *A_row_buffer = viennacl::linalg::host_based::detail::extract_raw_pointer<unsigned int>(
            vl_A.handle1());
    unsigned int const *A_col_buffer = viennacl::linalg::host_based::detail::extract_raw_pointer<unsigned int>(
            vl_A.handle2());
    vcl_scalar_type *A_elements = viennacl::linalg::host_based::detail::extract_raw_pointer<vcl_scalar_type>(
            vl_A.handle());

    //zero CSR vector in vl_A
    viennacl::vector_base<vcl_scalar_type> init_temporaryA(vl_A.handle(),
                                                           viennacl::compressed_matrix<vcl_scalar_type>::size_type(
                                                                   nnz_drift + 1), 0, 1);
    // write:
...
//setup the compressed matrix on the compute device, if available
    ...
    viennacl::linalg::chow_patel_icc_precond<viennacl::compressed_matrix<vcl_scalar_type> > deposition_flux_chow_patel_icc(
    ...
    viennacl::vector<vcl_scalar_type> vl_dSdt = viennacl::linalg::solve(vl_A, bb, deposition_flux_custom_cg,
                                                                        deposition_flux_chow_patel_icc);
    // viennacl::vector<vcl_scalar_type> vl_dSdt = viennacl::linalg::solve(vl_A, bb, deposition_flux_custom_cg);
