...
boost::shared_ptr<interpolation_base> operator()(std::string method)
{
    if(method == "idw")
    {
        return boost::make_shared<IDW>();
    }
    else if (method == "spline")
    {
        return boost::make_shared<spline>();        
    }
    else 
    {
...
//    }
  ...
  //  raster->set_raster_at(row,col,z0);
