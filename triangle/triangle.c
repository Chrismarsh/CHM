...
int triunsuitable(triorg, tridest, triapex, area, b)
...
//    //this is dumb. write to disk then re-read b/c not updating in memory, fix this later
//    OGR_DS_Destroy(DS);
//    GDALDatasetH tri   = GDALOpenEx("lol.shp",GDAL_OF_VECTOR,NULL,NULL,NULL);
//    OGRLayerH tri_layer = GDALDatasetGetLayerByName( tri, "lol" );
//    OGRFeatureH tri_feature;
//    OGR_L_ResetReading(tri_layer);
//    tri_feature = OGR_L_GetNextFeature(tri_layer);
...
//    printf("-------------------------\n");
    ...
    int px = (int)((mx - gt[0]) / gt[1]);  // x pixel
    int py = (int)((my - gt[3]) / gt[5]);  // y pixel
    if (px == GDALGetRasterBandXSize(dem_band))
        px = px -1;
    if (py == GDALGetRasterBandYSize(dem_band))
        py = py - 1;
    double tri_count = 0;
    pafScanline = (float *) CPLMalloc(sizeof(float)*1);
    GDALRasterIO( dem_band, GF_Read, px, py, 1, 1,
                  pafScanline, 1, 1, GDT_Float32,
                  0, 0 );
    if(pafScanline[0] != -9999.)
    {
        triangle_z_mean +=pafScanline[0];
        ++tri_count;
    }
...
//    }
    ...
    for (int i=0;i<ysize;i++)
    {
        ...
        //read the masked
        masked_pafScanline = (float *) CPLMalloc(sizeof(float)*nXSize);
...
#define STARTINDEX 1
    ...
    //if we find -T set this to true, so we can properly each the demfilename without screwing up triangle's expected input
    int expect_dem_filename = 0;
    ...
    for (i = STARTINDEX; i < argc; i++)
    {
            ...
            for (j = STARTINDEX; argv[i][j] != '\0'; j++)
            {
                ...
                if (argv[i][j] == 't')
                {
                        ...
                        (argv[i][j + 1] == '.'))
                    {
                        ...
                        workstring[k] = '\0';
                        b->maxtolerance = (REAL) strtod(workstring, (char **) NULL);
                        if (b->maxtolerance <= 0.0)
                        {
        ...
        {
            if(expect_dem_filename)
            {
                strncpy(b->demfilename, argv[i], FILENAMESIZE - 1);
                b->demfilename[FILENAMESIZE - 1] = '\0';
                expect_dem_filename=0;
            }
            else
            {
    ...
    if (b->demfilename[0] == '\0')
    {
        printf("demfilename is empty\n");
    }
    if ( b->demfilename[0] != '\0' && b->maxtolerance <=0)
    {
        printf("Demfilename give, but max tolerance is  <= 0.");
    }
    if ( b->demfilename[0] != '\0' && b->maxtolerance >0)
    {
...

{
    ...
    if (b->vararea || b->fixedarea || b->usertest)
    {
        ...
        if (b->usertest)
        {
            ...
            if (triunsuitable(torg, tdest, tapex, area,b))
            {
                enqueuebadtri(m, b, testtri, minedge, tapex, torg, tdest);
                return;
            }
